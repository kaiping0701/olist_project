...
CREATE TABLE olist_data_mart.fact_delivered_order_event(
    ...
    customer_location_key INT UNSIGNED NOT NULL,
    ...
    CONSTRAINT fk_fact_order_event_customer_location
        FOREIGN KEY(customer_location_key)
        REFERENCES olist_data_mart.dim_customer_location(customer_location_key),
...
WITH event_vw_order_agg AS(
    ...
    SELECT
        ob.order_event_key,
        MIN(dcl.customer_location_key) as customer_location_key,
        min(ob.customer_unique_id) as customer_unique_id, #每組order_event_key只有一種customer_key
        min(ob.event_purchase_timestamp)as event_purchase_timestamp,# event_purchase_ts取最小order_purchase_ts
        COUNT(*) as cnt_event_order,
        ROUND(AVG(ob.purchase_to_approval_seconds),0) as mean_purchase_to_approval_seconds,
        ROUND(AVG(ob.approved_to_carrier_seconds),0)  as mean_approved_to_carrier_seconds,
        ROUND(AVG(ob.carrier_to_customer_seconds),0)  as mean_carrier_to_customer_seconds,
        ROUND(AVG(ob.estimated_delivery_seconds),0)   as mean_estimated_delivery_seconds,
        ROUND(AVG(ob.delivery_seconds),0) as mean_delivery_seconds,
        -- 只取delay order 算平均延遲秒數
        -- AVG會忽略NULL因此delay_seconds<=0 不會參與計算
        -- 沒有延遲訂單紀錄為0
        COALESCE(ROUND(AVG(
                        CASE
                            WHEN ob.delay_seconds > 0 THEN ob.delay_seconds
                        END
                    ),0
                ),0) as mean_delay_seconds,
        MAX(ob.is_late_delivery) as has_late_delivery,
        SUM(ob.is_late_delivery) as cnt_late_order,
        SUM(ob.cnt_review) as cnt_review,
        MAX(ob.has_review) as has_review,
        SUM(ob.sum_review_score) as sum_review_score,
        ROUND(SUM(ob.sum_review_score) / NULLIF(SUM(ob.cnt_review),0),2) as mean_review_score,
        MAX(ob.has_low_review) as has_low_review,
        MAX(ob.has_comment) as has_comment,
        MAX(ob.max_review_comment_length) as max_review_comment_length,
        MAX(ob.has_positive) as has_positive,
        MAX(ob.has_negative) as has_negative,
        MAX(ob.has_mixed) as has_mixed,
        MAX(ob.has_neutral) as has_neutral,
        MAX(ob.has_t01_delivery_logistics) as has_t01_delivery_logistics,
        MAX(ob.has_t02_order_fulfillment) as has_t02_order_fulfillment,
        MAX(ob.has_t03_product_quality) as has_t03_product_quality,
        MAX(ob.has_t04_product_consistency) as has_t04_product_consistency,
        MAX(ob.has_t05_packaging_protection) as has_t05_packaging_protection,
        MAX(ob.has_t06_customer_service) as has_t06_customer_service,
        MAX(ob.has_t07_price_transaction) as has_t07_price_transaction,
        MAX(ob.has_t08_platform_system) as has_t08_platform_system,
        MIN(ob.first_review_creation_date) as first_review_creation_date,
        MAX(ob.last_review_answer_timestamp) as last_review_answer_timestamp,
        SUM(ob.cnt_payment_record) as cnt_payment_record,
        SUM(ob.payment_value) as payment_value,
        MAX(ob.max_payment_installments) as max_payment_installments,
        MAX(ob.has_credit_card) AS has_credit_card,
        MAX(ob.has_boleto) AS has_boleto,
        MAX(ob.has_voucher) AS has_voucher,
        MAX(ob.has_debit_card) AS has_debit_card,
        MAX(ob.has_not_defined) AS has_not_defined,

        (MAX(ob.has_credit_card)
        + MAX(ob.has_boleto)
        + MAX(ob.has_voucher)
        + MAX(ob.has_debit_card)
        + MAX(ob.has_not_defined)) AS cnt_distinct_payment_type,
        SUM(ob.cnt_item) as cnt_item,
        SUM(ob.sum_order_price) as sum_event_price,
        SUM(ob.sum_order_freight_value) as sum_event_freight_value,
        SUM(ob.order_total_value) as event_total_value,
        ROUND(SUM(ob.sum_order_freight_value) / NULLIF(SUM(ob.order_total_value),0),4) as freight_ratio,
        ROUND(
            SUM(ob.sum_order_price) / NULLIF(SUM(ob.cnt_item), 0),2
            ) AS mean_item_price,
        ROUND(MAX(ob.max_item_price),2) as max_item_price,
        ROUND(MIN(ob.min_item_price),2) as min_item_price

    FROM olist_data_mart.vw_delivered_order_base as ob
    INNER JOIN olist_data_mart.dim_customer_location as dcl
    ON ob.customer_zip_code_prefix = dcl.customer_zip_code_prefix
        AND ob.customer_city = dcl.customer_city
        AND ob.customer_state = dcl.customer_state
    GROUP BY ob.order_event_key
),
