...
WITH grain_check AS (
    ...
    SELECT
        'dim_customer_location_business_key',
        COUNT(*),
        COUNT(DISTINCT
    ...
    FROM olist_data_mart.dim_customer_location
...
/* =========================================================
   2. Source / Fact → Dimension / Map 關聯完整性
   ========================================================= */
...
WITH integrity_check AS (
    ...
    SELECT
        'source_customer_location_to_dim',
        COALESCE(SUM(dcl.customer_location_key IS NULL), 0)
    FROM (
        SELECT DISTINCT
            customer_zip_code_prefix,
            customer_city,
    ...
    LEFT JOIN olist_data_mart.dim_customer_location AS dcl
        ON src.customer_zip_code_prefix = dcl.customer_zip_code_prefix
       AND src.customer_city = dcl.customer_city
       AND src.customer_state = dcl.customer_state

    UNION ALL

    SELECT
        'fact_event_to_dim_customer_location',
        COALESCE(SUM(dcl.customer_location_key IS NULL), 0)
    FROM olist_data_mart.fact_delivered_order_event AS f
    LEFT JOIN olist_data_mart.dim_customer_location AS dcl
        ON f.customer_location_key = dcl.customer_location_key
...
event_location AS (
    SELECT
        m.order_event_key,
        MIN(dcl.customer_location_key)
            AS expected_customer_location_key,
        COUNT(DISTINCT dcl.customer_location_key)
            AS location_key_count,
        COALESCE(
            SUM(dcl.customer_location_key IS NULL),
            0
        ) AS unmapped_order_count
    FROM olist_data_mart.map_delivered_order_to_event AS m
    INNER JOIN olist_staging.stg_orders AS o
        ON m.order_id = o.order_id
    INNER JOIN olist_staging.stg_customers AS c
        ON o.customer_id = c.customer_id
    LEFT JOIN olist_data_mart.dim_customer_location AS dcl
        ON c.customer_zip_code_prefix = dcl.customer_zip_code_prefix
       AND c.customer_city = dcl.customer_city
       AND c.customer_state = dcl.customer_state
    GROUP BY m.order_event_key
),
map_event_check AS (
    SELECT
    ...
    SELECT
        'event_location_unresolved_or_multiple',
        COALESCE(SUM(
            location_key_count <> 1
            OR unmapped_order_count <> 0
        ), 0)
    FROM event_location
    ...
    SELECT
        'event_customer_location_mismatch',
        COALESCE(SUM(
            NOT (
                f.customer_location_key
                <=> el.expected_customer_location_key
            )
        ), 0)
    FROM event_location AS el
    INNER JOIN olist_data_mart.fact_delivered_order_event AS f
        ON el.order_event_key = f.order_event_key
)
SELECT
    test_name,
    violation_count,
...
SELECT
    COUNT(*) AS event_count,
    COUNT(DISTINCT customer_key) AS distinct_customer_count,
    COUNT(DISTINCT customer_location_key)
        AS distinct_customer_location_count,
